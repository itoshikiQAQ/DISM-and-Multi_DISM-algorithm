...
//Check all are numerical
    ...
    public Instances process(Instances data) throws Exception{
        ...
        double[] splits=new  double[data.numInstances()];
...
//Put into an order list
        ArrayList<OrderLineObj> list=new ArrayList<>();
        for(int i=0;i<vals.length;i++)
            list.add(new OrderLineObj(vals[i],classes[i]));
        //Sort the vals
        TreeMap<Double,Integer> tree = ShapeletTransform.getClassDistributions(data);
