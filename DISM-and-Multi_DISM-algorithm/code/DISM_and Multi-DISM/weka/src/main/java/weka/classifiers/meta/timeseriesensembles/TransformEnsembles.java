...
public class TransformEnsembles extends Classifier{
...
// this is just a bit of a hack to speed up experiments,
		if(rebuild){	
			...
			findWeights();
			all= Classifier.makeCopies(base,train.size());
                        all[0]=Classifier.makeCopy(baseTime);
	...
	public void findCVWeights() throws Exception {
		...
		for(int i=0;i<nosTransforms;i++){
			 Evaluation evaluation = new Evaluation(train.get(i));
                         if(i==0)
                            evaluation.crossValidateModel(Classifier.makeCopy(baseTime), train.get(i), folds, new Random());
                         else
                             evaluation.crossValidateModel(Classifier.makeCopy(base), train.get(i), folds, new Random());
