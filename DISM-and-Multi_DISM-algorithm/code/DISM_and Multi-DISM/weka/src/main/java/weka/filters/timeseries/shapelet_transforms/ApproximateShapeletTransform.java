...
public class ApproximateShapeletTransform extends ShapeletTransformDistCaching{
    ...
    private Instances sampleInstances(Instances data){
        if(seriesSampleLevel == 100){
            ...
            /* Used for testing
            TreeMap<Double, Integer> dist = ShapeletTransform.getClassDistributions(data);
            TreeMap<Double, Integer> dist2 = ShapeletTransform.getClassDistributions(sampledData);
            printTreeMap(dist);
            printTreeMap(dist2);
    ...
    private Instances performPAA(Instances data){
        ...
        if(dataPointsSize == 100){
            ...
            for(int i = 0; i < data.numInstances(); i++){ 
                ...
                Instance toAdd = new Instance(paaSize + 1);
                
                //Normalise series
                double[] series = currentInstance.toDoubleArray();
                series = ShapeletTransform.zNormalise(series, true);
    ...
    public double timingForSingleShapelet(Instances data, int minShapeletLength, int maxShapeletLength) throws Exception {
        ...
        output = QualityBound.roundRobinData(output);
    ...
    private double[] testPAA(double[] data) throws IOException{
        ...
        //Create instance
        Instance inst = new Instance(1, data);
        instances.add(inst);
    ...
    public static void main(String[] args){        
        ...
        ShapeletTransformOnlineNorm.printSeries(dataEven);
        ShapeletTransformOnlineNorm.printSeries(out);
    }
