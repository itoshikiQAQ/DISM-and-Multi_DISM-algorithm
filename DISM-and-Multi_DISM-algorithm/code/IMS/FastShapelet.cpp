...
int num_class, num_obj, subseq_len;//类的个数，样本的个数，子序列长度

Data_type       Org_Data, Data, NewData, ZData;
vector<int>     Org_Label, Label;
vector<int>     Org_Class_Freq, Class_Freq;//类出现的频率
int    num_obj_class;
...
void ReadTrainData(char* filename)
{
	...
	int class_id = 0, cur_class, real_class_id, tmp_class_id;//class_id应该是类的个数
...
void CreateSAXList(int subseq_len, int sax_len, int w)
{
	...
	//double sum_segment[sax_len];  // 修改
	//int elm_segment[sax_len];		// 修改	
	...
	for (obj_id = 0; obj_id<(int)NewData.size(); obj_id++)
	{
		ex = ex2 = 0;
		prev_word = -1;
		...
		/// Case 1: Initial
		for (j = 0; (j<(int)NewData[obj_id].size()) && (j<subseq_len); j++)
		{
			d = NewData[obj_id][j];
			ex += d;
			ex2 += d*d;
		...
		for (j = j; (j <= (int)NewData[obj_id].size()); j++)
		{
			j_st = j - subseq_len;
			...
			/// For next update
			if (j<(int)NewData[obj_id].size())
			{
				ex -= NewData[obj_id][j_st];
				ex2 -= NewData[obj_id][j_st] * NewData[obj_id][j_st];

				for (k = 0; k<sax_len - 1; k++)
				{
					sum_segment[k] -= NewData[obj_id][j_st + (k)*w];
					sum_segment[k] += NewData[obj_id][j_st + (k + 1)*w];
				}
				sum_segment[k] -= NewData[obj_id][j_st + (k)*w];
				sum_segment[k] += NewData[obj_id][j_st + min((k + 1)*w, subseq_len)];
...
//对shapelets从大到小排序
int sortShapelet(Shapelet s1, Shapelet s2)
{
	return (s1 > s2);
}
...
double calcAUC(vector<pair<double, double> > TPRFPRpairs)
{
	double auc = 0.0;
	double height = 0.0;
	double preX = 0.0;
	int num = TPRFPRpairs.size();
	for (size_t i = 0; i < num; i++)
	{
		if (TPRFPRpairs[i].first == preX)
		{
			height = TPRFPRpairs[i].second;
		}
		else
		{
			auc += height*(TPRFPRpairs[i].first - preX);
			preX = TPRFPRpairs[i].first;
		}
	}
	return auc;
}
...
vector<Shapelet> FindBestSAX(int top_k)
{
	vector<Shapelet> shapeletVector;
	Dist_List_type Dist(num_obj_class);
	SAX_word_type word;
	double auc, dist_th, gap;
	double maxf = 0.0;
	double maxdist_th = 0.0, maxgap = 0.0;
	...
	for (int k = 0; k < min(top_k, (int)Score_List.size()); k++)
	{
		...
		for (kk = 0; kk < min((int)usax.sax_id.size(), 1); kk++) {
			...
			for (int obj = 0; obj < num_obj_class; obj++)
			{
				dist = NearestNeighborSearch(query, NewData[obj], obj, Q, order);
			...
			////////////////////////////////////////////
			int setPositiveValue = Label[Dist[0].first];

			int Allpositives = 0;
			int Allnegtives = 0;
			for (size_t i = 0; i < num_obj_class; i++)
			{
				if (Label[i] == setPositiveValue)
				{
					Allpositives++;
				}
				else
			...
			vector<pair<double, double> > TPRFPRpairs;
			pair<double, double> init(0.0, 0.0);
			TPRFPRpairs.push_back(init);
			/////////////////////////////////////////
			total_c_in = 0;
			for (int i = 0; i < (int)Dist.size() - 1; i++)
			{
				...
				if (Dist[i].second <= dist_th)
				{
					//tp++;//预测的正类
					if (label == setPositiveValue) {
						ftp++;// 预测的正类中实际为正类的
					}
					else {
						ffp++;
				...
				//查准率,tp*1.0,这个1.0一定要加
				double p = ftp*1.0 / (ftp + ffp);
				//查全率
				double r = ftp *1.0 / Allpositives;
				f = 2 * 1.0 / (1 * 1.0 / p + 1 * 1.0 / r);



				//计算TPR和FPR
				double tpr = ftp*1.0 / Allpositives;
				double fpr = ffp*1.0 / Allnegtives;
				pair<double, double> temp(fpr, tpr);
				TPRFPRpairs.push_back(temp);
			...
			pair<double, double> end(1.0, 1.0);
			TPRFPRpairs.push_back(end);
			auc = calcAUC(TPRFPRpairs);

			double IR = (double)Allpositives / (double)Allnegtives;
			if (IR >= (1.0 / 3.0) && IR <= 3.0) {
		                           		avg = maxf*0.5 + auc*0.5;
			}
			else {
				...
				{
					double p = 1.0 / IR;
					avg = maxf*(1.0 / p) + auc*((p - 1) / p);
				}
				else
				{
					avg = maxf*(1.0 / IR) + auc*((IR - 1) / IR);
				}

			}

			sh.setValueFew(avg, maxgap, maxdist_th);
			//如果当前的shapelet 的gap大于bsf的gap，那么将此条记录赋值给bsf_sh
			if (bsf_sh < sh)
			{
				bsf_sh.setValueAll(avg, maxgap, maxdist_th, q_obj, q_pos, subseq_len, num_diff, c_in, c_out);
			}


		}
		shapeletVector.push_back(bsf_sh);
...
int main(int argc, char* argv[])
{
	...
	/********************************/
	if (argc < 7)    error(0);
	train_file_name = argv[1]; //训练集文件名称
	num_class = atoi(argv[2]); //类的个数
	num_obj = atoi(argv[3]);   //训练集中时间序列的数量
	max_len = atoi(argv[4]);   //设置shapelets最大长度
	min_len = atoi(argv[5]);   //设置shapelets最小长度
	step = atoi(argv[6]);   //设置步长，一般设为1
	...
	vector<Shapelet> allLengthBestShapelet;		//新添加
	vector<Shapelet> oneLengthBestShapelet;
	/// Find Shapelet
	...
	for (int h = 0; h < num_class; h++) {
		for (int j = h + 1; j < num_class; j++)
		{
			//int a = Class_Freq[h];
			//int b = Class_Freq[j];
			num_obj_class = Class_Freq[h] + Class_Freq[j];
			for (int w = 0; w < num_obj; w++) {
				//int a = Label[w];
				//int b = Real_Class_Label[h];
				//int c = Real_Class_Label[j];
				if (Label[w] == h || Label[w] == j) {
					NewData.push_back(Data[w]);//新的二分类数据组
											   //printf("%d", w);
				}
			}
			...
			for (subseq_len = min_len; subseq_len <= max_len; subseq_len += step)
			{
				...
				oneLengthBestShapelet = FindBestSAX(top_k);
				...
				{
					allLengthBestShapelet.push_back(oneLengthBestShapelet.at(num));
				}
	...
	sort(allLengthBestShapelet.begin(), allLengthBestShapelet.end(), sortShapelet);
	vector<Shapelet>::iterator it;
	it = unique(allLengthBestShapelet.begin(), allLengthBestShapelet.end());
	allLengthBestShapelet.erase(it, allLengthBestShapelet.end());
	//输出shapelets的文件名格式
	char shapeletsOutFileName[80];
	strcpy(shapeletsOutFileName, argv[1]);
	strcat(shapeletsOutFileName, "_maxlen");
	strcat(shapeletsOutFileName, argv[4]);
	strcat(shapeletsOutFileName, "_minlen");
	strcat(shapeletsOutFileName, argv[5]);
	strcat(shapeletsOutFileName, ".txt");

	FILE* f_shapeletResult = fopen(shapeletsOutFileName, "w");
	fprintf(f_shapeletResult, "-------------------all shapelets-------------\n");
	fprintf(f_shapeletResult, "   obj	      pos	    	length	    	avg		   gap		   distanceThreshold	\n");
	for (int shapeletNum = 0; shapeletNum < allLengthBestShapelet.size(); shapeletNum++)
	{
		fprintf(f_shapeletResult, "%3d 	    %8d 	%8d 	  %8.6f 	     %8.6f 		  %8.6f\n",
			allLengthBestShapelet.at(shapeletNum).obj,
			allLengthBestShapelet.at(shapeletNum).pos,
			allLengthBestShapelet.at(shapeletNum).len,
			allLengthBestShapelet.at(shapeletNum).gain,
			allLengthBestShapelet.at(shapeletNum).gap,
			allLengthBestShapelet.at(shapeletNum).dist_th);

	}
	fclose(f_shapeletResult);
	/*if (bsf_sh.len <= 0)
